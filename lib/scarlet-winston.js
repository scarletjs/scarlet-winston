...
var ScarletWinston = module.exports = exports = function(scarlet){
	...
	self.beforeMethodCall = function(invocation){
		...
		var time = new Date();
		var message = "["+time+"] "+"calling - "+invocation.contextName()+"::"+invocation.memberName()+"("+formatter.formatArgs(invocation.args)+")";
	...
	self.afterMethodCall = function(invocation){
		...
		var time = new Date();
		var message = "["+time+"] "+invocation.contextName()+"::"+invocation.memberName()+"("+formatter.formatArgs(invocation.args)+") - returned:"+JSON.stringify(invocation.result)+" - execution time("+formatter.getFormatedTimeSpan(invocation.executionStartDate,invocation.executionEndDate)+")";
		self.winston.log("info",message);
	};
